...
const RecurringTransactionDetail = ({
  ...
  const hasEntryForNextDate = historyData.some((h) =>
    dayjs(h.date).isSame(dayjs(recurring.nextDate), "day"),
  );
  ...
  const latestSnooze = historyData.find(
    (h) => h.status === "snoozed",
  )?.snoozedUntil;
    ...
    <>
      <Modal
        ...
        footer={[
          <Button
            key="skip"
            onClick={() => setConfirmAction("skipped")}
            disabled={loading || hasEntryForNextDate || isFutureDate}
          >
            Skip
          </Button>,
          <Button
            key="snooze"
            onClick={() => setConfirmAction("snoozed")}
            disabled={loading || hasEntryForNextDate || isFutureDate}
          >
            Snooze
          </Button>,
          <Button
            ...
            loading={loading}
            disabled={hasEntryForNextDate || isFutureDate}
          >
            Mark Paid
          </Button>,
        ]}
      ...
      >
        <p>
          <b>Category:</b> {recurring.category}
        </p>
        <p>
          <b>Amount:</b> ₹{recurring.latestAmount}
        </p>
        <p>
          <b>Frequency:</b> {recurring.frequency}
        </p>
        <p>
          <b>Next Date:</b> {dayjs(recurring.nextDate).format("DD MMM YYYY")}
        </p>
        {latestSnooze && (
          <p>
            <b>Last Snoozed Until:</b>{" "}
            {dayjs(latestSnooze).format("DD MMM YYYY")}
          </p>
        )}
