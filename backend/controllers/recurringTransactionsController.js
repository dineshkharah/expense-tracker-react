...
const createRecurring = async (req, res) => {
  try {
    const { source, category, amount, type, startDate, frequency, notes } =
      req.body;
    ...
    const recurring = await RecurringTransaction.create({
      ...
      category,
      amounts: [
        { amount: encryptedAmount, effectiveFrom: new Date(startDate) },
      ],
      type,
      startDate,
      nextDate,
    ...
    res.status(201).json({
      ...recurring._doc,
      latestAmount: decrypt(
        recurring.amounts[recurring.amounts.length - 1].amount,
      ),
    });
  } catch (error) {
    console.error(error);
...
const executeRecurring = async (req, res) => {
  try {
    ...
    if (!recurring) {
      return res
        .status(404)
        .json({ message: "Active recurring rule not found" });
    }

    const targetedPeriodEnd = periodEnd
      ? new Date(periodEnd)
    ...
    if (action === "paid") {
      ...
      if (alreadyProcessedForPeriod) {
        return res
          .status(400)
          .json({ message: "Transaction already paid for this period" });
      }
